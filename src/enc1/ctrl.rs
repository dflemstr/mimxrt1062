...
impl From<CMPIE_A> for bool {
    ...
    fn from(variant: CMPIE_A) -> Self {
        match variant {
            CMPIE_A::CMPIE_0 => false,
            CMPIE_A::CMPIE_1 => true,
        }
    }
}
...
impl From<CMPIRQ_A> for bool {
    ...
    fn from(variant: CMPIRQ_A) -> Self {
        match variant {
            CMPIRQ_A::CMPIRQ_0 => false,
            CMPIRQ_A::CMPIRQ_1 => true,
        }
    }
}
...
pub enum PH1_A {
    ...
    PH1_0,
    #[doc = "1: Bypass the quadrature decoder. A positive transition of the PHASEA input generates a count signal. The PHASEB input and the REV bit control the counter direction. If CTRL\\[REV\\] = 0, PHASEB = 0, then count up If CTRL\\[REV\\] = 0, PHASEB = 1, then count down If CTRL\\[REV\\] = 1, PHASEB = 0, then count down If CTRL\\[REV\\] = 1, PHASEB = 1, then count up"]
    PH1_1,
}
