...
pub enum TST_A {
    ...
    TST_1,
    #[doc = "2: Watchdog test mode enabled, only the low byte is used. CNT\\[CNTLOW\\] is compared with TOVAL\\[TOVALLOW\\]."]
    TST_2,
    #[doc = "3: Watchdog test mode enabled, only the high byte is used. CNT\\[CNTHIGH\\] is compared with TOVAL\\[TOVALHIGH\\]."]
    TST_3,
}
...
impl From<UPDATE_A> for bool {
    ...
    fn from(variant: UPDATE_A) -> Self {
        match variant {
            UPDATE_A::UPDATE_0 => false,
            UPDATE_A::UPDATE_1 => true,
        }
    }
}
...
impl From<CMD32EN_A> for bool {
    ...
    fn from(variant: CMD32EN_A) -> Self {
        match variant {
            CMD32EN_A::CMD32EN_0 => false,
            CMD32EN_A::CMD32EN_1 => true,
        }
    }
}
